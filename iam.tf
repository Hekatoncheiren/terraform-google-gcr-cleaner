...
resource "google_artifact_registry_repository_iam_member" "this" {
  ...
  repository = "projects/${each.value.project_id != null ? each.value.project_id : local.google_project_id}/locations/${each.value.region}/repositories/${split("/", each.value.name)[0]}"
